...
| Enquiry + dealer forms → Web3Forms/Formspree → email; WhatsApp deep links | User accounts, logins, dashboards | Dealer portal (locator, lead tracking) |
...
| 0.2 | Label art per SKU | Vector label layouts for `def-5l`, `def-10l`, `def-20l`, `def-210l`, `def-1000l` (+ generic bulk visual for `def-bulk`); must carry ISO 22241 / IS 17042 references, batch/expiry zones, GST/address block. BIS mark placeholder until CM/L number confirmed [CONFIRM: BIS licence number — see Dependency map] |
| 0.3 | Blender packaging models | Model 5 pack forms (can ×2 sizes share topology, bucket, drum, IBC); UV-mapped for real label art; export GLB + Draco + KTX2 to design.md §6.1 per-model caps — cans ≤ 400 KB, `def-20l` hero bucket ≤ 450 KB, drum ≤ 350 KB, IBC ≤ 900 KB; hero scene total ≤ 1.2 MB (model + label/material atlas) — these caps protect design.md §8.2 page-weight budgets; light with Poly Haven studio HDRI |
| 0.4 | Render set | Blender Cycles hero renders + per-SKU renders (AVIF posters at multiple sizes — these are the LCP images and the lite/static-tier visuals) |
| 0.5 | Stock & AI asset gathering | Indian truck/tractor/farm/industrial/lab imagery per research/research-assets.md queries; FLUX blank-label scene renders with composited label art where stock has gaps; 8–12 s hero video loops ≤3 MB where used; avoid visible truck-maker logos and recognizable faces |
...
| 0.7 | Copy sign-off | Final content.md copy approved by client — especially claims: "5+ years", delivery promises (24/7 / 48 h / emergency), capacity/districts stats [CONFIRM: each claim operationally true] |
| 0.8 | Client confirmations round | Chase the brief's 7 open questions in one structured call; record answers; update placeholders list |
| 0.9 | PDF datasheet v1 | Shared ISO 22241-1 / IS 17042 spec table as branded downloadable PDF (linked from `/products#spec-table`) |

**Deliverables:** `/assets` library (SVG logos, label vectors, GLB models, AVIF poster/render sets, stock photo/video picks), licence log, signed-off copy doc, datasheet PDF, confirmations record.

**Acceptance criteria:** every Phase-2 section has at least a placeholder-quality visual assigned; all assets licence-logged; GLBs load in a three.js viewer within the design.md §6.1 per-model caps (≤ 350–900 KB depending on pack; hero scene ≤ 1.2 MB total); copy contains zero forbidden terms; unresolved client questions carried as explicit [CONFIRM] markers, not guesses.
...
| 2.7 | Contact `/contact` | `#reach` (tap-to-call phones +91 84495 07181 / +91 88002 54537, WhatsApp deep link with prefilled message, emails, hours — email rows flag-gated per §4 row 10, hours row ships only once confirmed per §4 row 12), `#form` (name, phone required, need-type dropdown: Buy DEF / Become dealer / Bulk enquiry / Other, message), `#map` (Bulandshahr embed, lazy-loaded), `#registered` (address + GST) |
| 2.8 | Forms wiring | Web3Forms or Formspree free tier [decide at build: Web3Forms preferred — no dashboard account needed for client]; honeypot + time-trap spam protection; success/error states without JS where possible (native POST + redirect) and enhanced with JS; forwarding to founder's inbox until domain mail exists (address itself never shown on site) |
...
|---|---|---|---|
| 4.1 | **Hero Three.js island** (`/#hero`) | One vanilla Three.js island; GLB/Draco pack model + liquid particle field in `--def-400`; scroll-rotated; AVIF poster is the LCP with `fetchpriority="high"`; canvas mounts post-idle (`requestIdleCallback`) on full tier only; shader warm-up before visibility; `<noscript>` image; renderer paused when off-screen | 3–4 d |
...
| 4.3 | **Product pack selector** (`/products#selector`, home `#products` rail) | SKU switch swaps 3D model (full tier) or AVIF render (lite/static) with liquid-fill transition; updates capacity/fitment/buyer copy per SKU id; all six SKUs incl. `def-bulk` generic visual; deep-linkable from per-SKU anchors | 2–3 d |
...
| 5.7 | Content freeze + proofread | Final copy pass against master brief terminology; resolve or consciously ship each remaining [CONFIRM] with soft wording |
...
| # | Task | Notes |
|---|---|---|
| 6.1 | Production hosting | Production project on chosen host (Vercel/Netlify/Cloudflare Pages); preview → production promotion flow; deploy from `main` only |
...
| 6.3 | Domain emails | info@ / sales@ / dealers@apexdef.in — **client creates these at the registrar / Google Workspace / Zoho** [CONFIRM: which email provider the client will use]; dev supplies exact MX/SPF/DKIM/DMARC records to add; until live, forms forward to founder's Gmail (never displayed on site). **Hard go-live gate (§4 row 10):** if the mailboxes are not live and receiving at launch, the email rows (footer + `/contact#reach`) are hidden behind a config flag — mirroring the social-icons pattern — rather than printing dead addresses |
...
| 6.5 | WhatsApp click-to-chat verification | Both numbers (+91 84495 07181, +91 88002 54537) verified receiving via `wa.me` links with prefilled messages, from Android + iPhone, in-app browser included [CONFIRM: which number is primary for WhatsApp Business] |
| 6.6 | Analytics | Plausible or GA4 [decide with client: Plausible = lighter/cookieless (fits perf budget); GA4 = free]; custom events wired: tel: clicks, wa.me clicks, form submits (per form), PDF datasheet downloads, pack-selector interactions |
...
**Acceptance criteria:** no [CONFIRM] placeholder older than one quarter without a client decision; norms copy never stale past a quarter; CWV field data (CrUX) stays green.
...
References the master brief's open-questions list (§7) plus the additional client inputs consolidated in content.md §6. Rule: **no dependency ever stops the build** — each has a "proceed with meanwhile" path. All placeholders tracked in a single PLACEHOLDERS.md in the repo, which mirrors content.md §6's full consolidated [CONFIRM] list; the table below tracks the build-blocking subset with needed-by phases.
...
| 2 | **Final pack lineup beyond 20 L** (5/10/210/1000/bulk?) | Which SKU cards/anchors ship; Blender model set; selector entries; label art count | Phase 0 modeling; Phase 2 `/products` | Build all six per locked lineup (`def-5l`…`def-bulk`) since `def-20l` is confirmed and others are marked assumed; hide unconfirmed SKU cards behind a one-line config flag if client trims the range |
| 3 | **"5+ years experience", founding year, capacity, districts served** | `#stats` counter values, `/about#story`, `#opportunity` pitch numbers | Phase 2 copy; Phase 5 freeze | Use structure with placeholder-safe phrasing ("lab-tested every batch", "48 h delivery") that needs no unverified numbers; counters marked [CONFIRM] render as qualitative chips until numbers land. **Ask the capacity question in one unit only — litres/day** (content.md `#stats` counter 2 is "litres of daily production capacity"; design.md must use the same denomination) |
...
| 5 | **Actual product/label photos, or proceed with 3D renders from label art?** | Whether Phase 0 render set is interim or final; photo shoot scheduling | Phase 0 | Proceed with Blender renders from label art as the plan of record (brief's art direction already prefers this); slot a half-day Bulandshahr plant/product shoot as a Phase 7 upgrade |
| 6 | **Delivery promises to keep** (24/7? 48 h? emergency?) | `#stats`, `#why-apex`, `/products#how-to-buy`, Dealers pitch — every delivery claim | Phase 2 copy; Phase 5 freeze (hard gate: claims must be operationally true) | Draft with the brochure claims flagged [CONFIRM] each; if unconfirmed at freeze, downgrade to "fast doorstep delivery across [region] — call to confirm timelines" |
| 7 | **Social media handles** (link existing / create fresh?) | Footer social icons; Organization JSON-LD sameAs | Phase 2 footer; Phase 6 | Ship footer without social row (cleaner than dead links); add icons + sameAs when handles exist |
| 8 | **Label art source files** (from client or created in Phase 0.2?) | Blender UV texturing (0.3), renders (0.4), datasheet branding | Phase 0 | Design label art in-house in 0.2 from brief facts (address, GST, standards); client approves before render batch [CONFIRM: any existing label design to match?] |
| 9 | **Plant photos** (existing photos of Bulandshahr facility?) | `/about#plant`, `#quality-lab` imagery | Phase 2 | Generic-but-honest industrial stock + AI scene renders per research/research-assets.md, captioned carefully (never claim a stock photo is "our plant" — use process-illustrative captions); replace via Phase 7 shoot |
...
| 11 | **Exact registered legal entity name** ("Apex Environment Solutions" — proprietorship/LLP/Pvt Ltd?) (content.md §6 item 2 — content-declared launch blocker) | Footer © line, `/contact#registered` block, Organization `legalName` in JSON-LD | Phase 2 (schema + footer); Phase 5 freeze | Display brand line "Apex Environment Solutions" as-is; omit `legalName` from JSON-LD and mark the © line [CONFIRM] until the registered name is confirmed — never guess a suffix |
| 12 | **Business hours + response-time commitment** (content.md §6 item 13) | `/contact#reach` hours row, `openingHours` in ContactPage schema, form success-message response promise | Phase 2 copy; Phase 5 freeze | Ship `#reach` without the hours row and omit `openingHours` from schema until confirmed; response promise uses soft wording ("we respond within one working day [CONFIRM]") |
...
| 14 | **MOQ / retail route for single cans** (content.md §6 item 5) | Products FAQ 2, Home FAQ 5, `def-5l` CTA line | Phase 2 copy; Phase 5 freeze | Ship content.md's soft wording ("tell us what you need — we'll route it sensibly") flagged [CONFIRM]; firm up MOQ numbers only when confirmed |
...
- [ ] All remaining [CONFIRM] items resolved or downgraded to soft wording, with client sign-off recorded
...
| 2 | **Client asset delays** (label art, photos, confirmations) | High | Medium (schedule) | Dependency map (§4) gives every input a "proceed meanwhile" path; Blender renders + licensed stock + AI scene renders cover all imagery; PLACEHOLDERS.md + 5-day escalation rule prevents silent slippage |
...
| 8 | **Unverifiable claims shipped** (delivery promises, experience years, capacity) | Medium | Medium (trust/legal) | Every numeric/service claim gated behind [CONFIRM]; Phase 5.7 freeze downgrades unconfirmed claims to soft wording; claims register kept alongside PLACEHOLDERS.md |
...
- If Plausible is chosen: use custom events + props (cookieless, no consent banner needed, lighter script — fits the perf budget). If GA4: serve via minimal gtag, `defer`, and accept the ~50 KB cost on full/lite tiers only [decide with client at 6.6].
